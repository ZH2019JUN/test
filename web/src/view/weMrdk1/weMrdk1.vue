  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="center" label="学号" prop="xh" width="130" />
        <el-table-column align="left" label="姓名" prop="name" width="130" />
        <el-table-column align="center" label="目前居住地" prop="szdq" width="160" />
        <el-table-column align="center" label="是否与湖北人员接触" prop="sfhxdgr" width="150" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateWeMrdk1Func(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getWeMrdk1List({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteWeMrdk1ByIds({ log_id })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateWeMrdk1Func = async(row) => {
  const res = await findWeMrdk1({ logId: row.logId })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteWeMrdk1Func = async(row) => {
  const res = await deleteWeMrdk1({ logId: row.logId })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
